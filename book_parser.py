...
def download_txt(book_id, filename, media_folder):
    """Download text files.
...
def parse_book_page(response):
    """Parse book webpage for book info, comments and cover link."""
    ...
    return book_title, book_author, image_link, comments, genres
...
def main():
    ...
    for book_id in range(args.start_id, args.end_id + 1):
        try:
            ...
            title, author, image_link, comments, genres = parse_book_page(book_page)
            filename = f"{book_id}. {title}"
            download_txt(book_id, filename, book_folder)
